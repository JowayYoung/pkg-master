...
module.exports = async function() {
	const question = [{
		...
		validate(val) {
			...
			if (!val) {
				return Chalk.redBright(CREATE_TEXT.judgeAuthorEmpty);
	...
	Fs.writeFileSync(packageDP, JSON.stringify(packageJson, null, "\t"), "utf8");
	for (const v of ReadFile("../temp", 1)) {
	...
	spinner.stop();
	console.log(Chalk.greenBright(CREATE_TEXT.newed));
};